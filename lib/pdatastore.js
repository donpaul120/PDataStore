 ...
 *              @example : <div id='p-store' data-source1='json_string' data-source2='json_string'></div>
 ...
 *
 *              Please note that PDataStore requires that the dataSource is a valid json format, if not
 *              PDataSore will throw a parse exception.
 * @constructor
 */
...
PDataStore.prototype.retrieveData = function(attr){
    ...
    if(this.pCache.hasOwnProperty(attr)){
        ...
        if(this.isJson(attrVal)){
            ...
            //we need to perform a request here...
...
PDataStore.prototype.findDataByKeyValue = function(k, v, attr){
    ...
    console.log("dataSource "+ attr + " started at:" +new Date().getUTCMilliseconds());
    ...
    //(for test purpose)calculating the time it takes to iterate through n items
    console.log("dataSource "+ attr + " stopped at:" +new Date().getUTCMilliseconds() + ", for = "+data.length + " items");
    this.result = result;
    return this;
};
